...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-nos-700 dark:bg-nos-900">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center space-x-2 me-5 rtl:space-x-reverse" wire:navigate>
            <a href="{{ route('dashboard') }}" class="flex items-center space-x-2 me-5 rtl:space-x-reverse" wire:navigate>
                @if (session()->has('company_id') && ($company = App\Models\Company::find(session('company_id'))))
                    <div
                        class="flex items-center justify-center rounded-md aspect-square size-8 bg-accent-content text-accent-foreground">
                        @if ($company->logo)
                            <img src="{{ asset('storage/' . $company->logo) }}" alt="{{ $company->name }}"
                                class="rounded-md size-8">
                        @else
                            <span
                                class="flex items-center justify-center w-full h-full text-white rounded-md bg-primary-600 dark:bg-primary-500">
                                {{ $company->initials }}
                            </span>
                        @endif
                    </div>
        ...
        <flux:navlist variant="outline">
            ...
            <flux:navlist.group :heading="__('Companies')" class="grid">
                <flux:navlist.item icon="building-office" :href="route('companies.index')"
                    :current="request()->routeIs('companies.index')" wire:navigate>{{ __('List of Companies') }}
                </flux:navlist.item>
                <flux:navlist.item icon="plus" :href="route('companies.create')"
                    :current="request()->routeIs('companies.create')" wire:navigate>{{ __('Create a Company') }}
                </flux:navlist.item>
            </flux:navlist.group>
            @if (session()->has('company_id'))
                <flux:navlist.group :heading="__('Departments')" class="grid">
                    <flux:navlist.item icon="users" :href="route('departments.index')"
                        :current="request()->routeIs('departments.index')" wire:navigate>
                        {{ __('List of Departments') }}</flux:navlist.item>
                    <flux:navlist.item icon="plus" :href="route('departments.create')"
                        :current="request()->routeIs('departments.create')" wire:navigate>
                        {{ __('Create a Department') }}</flux:navlist.item>
                </flux:navlist.group>

                <flux:navlist.group :heading="__('Designations')" class="grid">
                    <flux:navlist.item icon="users" :href="route('designations.index')"
                        :current="request()->routeIs('designations.index')" wire:navigate>
                        {{ __('List of Designations') }}</flux:navlist.item>
                    <flux:navlist.item icon="plus" :href="route('designations.create')"
                        :current="request()->routeIs('designations.create')" wire:navigate>
                        {{ __('Create a Designation') }}</flux:navlist.item>
                </flux:navlist.group>

                <flux:navlist.group :heading="__('Employees')" class="grid">
                    <flux:navlist.item icon="users" :href="route('employees.index')"
                        :current="request()->routeIs('employees.index')" wire:navigate>{{ __('List of Employees') }}
                    </flux:navlist.item>
                    <flux:navlist.item icon="plus" :href="route('employees.create')"
                        :current="request()->routeIs('employees.create')" wire:navigate>{{ __('Create an Employee') }}
                    </flux:navlist.item>
                </flux:navlist.group>

                <flux:navlist.group :heading="__('Contracts')" class="grid">
                    <flux:navlist.item icon="users" :href="route('contracts.index')"
                        :current="request()->routeIs('contracts.index')" wire:navigate>{{ __('List of Contracts') }}
                    </flux:navlist.item>
                    <flux:navlist.item icon="plus" :href="route('contracts.create')"
                        :current="request()->routeIs('contracts.create')" wire:navigate>{{ __('Create a Contract') }}
                    </flux:navlist.item>
                </flux:navlist.group>
